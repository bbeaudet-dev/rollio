...
import { RoundManager } from '../../game/engine/RoundManager';
import { calculatePreviewScoring, processCompleteScoring, processReroll } from '../../game/logic/gameActions';
...
export class WebGameManager {
  ...
  // Perform the first roll of a round (called when user clicks Roll button)
  performFirstRoll(state: WebGameState): WebGameState {
    if (!state.gameState || !state.roundState) return state;
    
    const newGameState = { ...state.gameState };
    const newRoundState = { ...state.roundState };
    
    // If diceHand is empty, populate it with the dice set first
    if (newRoundState.diceHand.length === 0) {
    ...
    newRoundState.rollHistory.push({
      rollNumber: 1,
      isReroll: false,
      diceHand: newRoundState.diceHand.map((d: any) => ({ ...d })),
      selectedDice: [],
      rollPoints: 0,
      maxRollPoints: 0,
      scoringSelection: [],
    ...
    // Check for reroll prompt (BEFORE flop check)
    ...
    if (newGameState.currentLevel.rerollsRemaining && newGameState.currentLevel.rerollsRemaining > 0) {
      ...
      this.gameInterface.askForReroll(newRoundState.diceHand, newGameState.currentLevel.rerollsRemaining);
      return this.createWebGameState(newGameState, newRoundState, [], null, [], [], false, false, false);
    }

    // Check for immediate flop (only after reroll prompt is handled)
    return this.checkForFlopAfterRoll(newGameState, newRoundState);
  }

  // Check for flop after a roll (handles flop shield choice)
  private checkForFlopAfterRoll(gameState: GameState, roundState: RoundState): WebGameState {
    if (isFlop(roundState.diceHand)) {
      ...
      // Set pending action for dice selection
  ...
  handleRerollSelection(state: WebGameState, selectedIndices: number[]): WebGameState {
    ...
    this.rollManager.rollDice(diceToReroll);
  ...
  async useConsumable(state: WebGameState, index: number): Promise<WebGameState> {
    ...
    await applyConsumableEffect(index, gameState, roundState, this.gameInterface, this.charmManager);
